...
export default function TempleDirectory({
  ...
  // Normalize temples (Fix missing states)
  ...
  const states = useMemo(() => {
    return ['All States', ...INDIAN_STATES];
  }, []);
  ...
  const filteredTemples = useMemo(() => {
    return normalizedTemples.filter((temple) => {
      ...
      const matchState = selectedState === 'All States' || temple.state === selectedState;
      ...
      return matchQuery && matchState && matchRating && isOpen;
    });
  }, [normalizedTemples, searchQuery, selectedState, minRating, openNowOnly]);
  ...
  const sortedStates = useMemo(() => Object.keys(groupedTemples).sort(), [groupedTemples]);

  return (
    <div className="space-y-8">
      {/* Premium Filter Header */}
      <div className="sticky top-20 z-40 -mx-4 md:mx-0 px-4 md:px-0">
        <div className="bg-white/80 backdrop-blur-2xl p-4 md:p-6 rounded-[2rem] border border-white/60 shadow-[0_20px_50px_-20px_rgba(0,0,0,0.15)] flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="relative group min-w-[160px]">
              ...
              <span className="absolute right-4 top-1/2 -translate-y-1/2 material-symbols-outlined pointer-events-none text-primary/40">expand_more</span>
            </div>
            ...
            >
              <span className={`w-2 h-2 rounded-full animate-pulse ${openNowOnly ? 'bg-white' : 'bg-emerald-500'}`} />
              Open Now
            </button>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between px-2">
        ...
        {(searchQuery || selectedState !== 'All States' || minRating > 0 || openNowOnly) && (
          <button 
            onClick={() => {
              setSearchQuery('');
        ...
        {sortedStates.length === 0 ? (
          <div className="text-center py-32 bg-white rounded-[3rem] border border-outline-variant/10 shadow-sm animate-in fade-in zoom-in duration-700">
            <div className="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-6">
              <span className="material-symbols-outlined text-5xl text-primary/40">temple_hindu</span>
            </div>
            <h3 className="text-2xl font-serif font-black text-secondary mb-2">No Temples Found</h3>
          ...
          sortedStates.map((state) => (
            <section key={state} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  <h2 className="text-3xl font-serif font-black text-secondary tracking-tight">{state}</h2>
                  ...
                  >
                    ...
                    <div className="relative h-56 overflow-hidden">
                      {temple.photos?.[0] ? (
                        ...
                        <div className="w-full h-full bg-surface-container-low flex items-center justify-center">
                          <span className="material-symbols-outlined text-6xl text-primary/20">temple_hindu</span>
                        </div>
                      )}
                      ...
                      <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md rounded-2xl px-3 py-1.5 flex items-center gap-1 shadow-xl border border-white/50">
                        <span className="material-symbols-outlined text-primary text-sm font-black" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                      ...
                      <div className="absolute bottom-4 left-4">
                        <div className={`px-4 py-1.5 rounded-full backdrop-blur-md border shadow-lg flex items-center gap-2 ${
                          isTempleOpen(temple.timings?.open, temple.timings?.close)
                            ? 'bg-emerald-500/80 text-white border-emerald-400/50'
                            : 'bg-red-500/80 text-white border-red-400/50'
                        }`}>
                          <div className={`w-1.5 h-1.5 rounded-full ${
                             isTempleOpen(temple.timings?.open, temple.timings?.close) ? 'bg-white animate-pulse' : 'bg-white/50'
                          }`} />
                          <span className="text-[10px] font-black uppercase tracking-widest">
                            {isTempleOpen(temple.timings?.open, temple.timings?.close) ? 'Open Now' : 'Closed'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-2 text-on-surface-variant/70 mb-4">
                        ...
                        <span className="text-sm font-bold truncate">{temple.city}</span>
                      </div>
